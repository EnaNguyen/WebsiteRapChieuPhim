...
namespace ProjectGSMVC.Areas.Admin.Controllers
{
    ...
    public class PhimController : Controller
    {
        ...
        public async Task<IActionResult> CreateMovie([FromBody] CreateMovie model)
        {
            ...
            if (model.PhimDatas.ThoiLuong <= 0)
            {
                errors["ThoiLuong"] = "Thời lượng phim phải lớn hơn 0.";
            }
            if (model.PhimDatas.GioiHanDoTuoi <= 0)
            {
                errors["GioiHanDoTuoi"] = "Giới hạn độ tuổi phải lớn hơn 0.";
            }
            if (model.PhimDatas.SoSuatChieu < 0 || model.PhimDatas.SoSuatChieu > 42)
            {
                errors["SoXuatChieu"] = "Số xuất chiếu phải lớn hơn 0 và nhỏ hơn 42 trong 1 tuần";
            }
            if(model.PhimDatas.NgayKhoiChieu<DateOnly.FromDateTime(DateTime.Now))
            {
                errors["NgayKhoiChieu"] = "Ngày khởi chiếu phải bắt đầu sau hôm nay";
            }
            if(model.PhimDatas.NgayKetThuc < model.PhimDatas.NgayKhoiChieu)        
            {
                errors["NgayKetThuc"] = "Phim phải chiếu được ít nhất 1 ngày";
            }    
            if (model.HinhAnhs.Count < 1)
            {
                errors["HinhAnhs"] = "Hình ảnh không được để trống.";
            }
